...
Johnson's relative weights transform the raw regression coefficients by considering the correlation matrix of the predictors. The result is a set of weights that reflect the contribution of each predictor to the model’s predictive power, adjusted for overlaps due to correlations among predictors. 

Johnson's weights provide a more nuanced view of feature importance than simple correlation or regression coefficients alone, particularly useful in models where predictors are not independent.
...
shapley_df = pd.DataFrame({
    'Feature': X_unstandardized.columns,
...
sns.heatmap(merged_df, annot=True, cmap='cividis', fmt=".2f")
